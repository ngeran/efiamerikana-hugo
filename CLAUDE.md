...
**Decap CMS (`site/static/admin/`):** `index.html` loads the editor from CDN; `config.yml` configures a GitHub backend (PKCE — the `backend.app_id` must be filled in with a GitHub OAuth App client_id) and three collections: a `file` collection editing `content/_index.md`, and `folder` collections for Videos and Gallery whose fields match the frontmatter (including the `build` headless object, via a YAML anchor, so CMS edits preserve headless behavior). `local_backend: true` lets you edit locally via `npx decap-server` at `/admin/`.
...
- **Cloudflare Pages (public):** `just cf` runs `nix build .#site` and uploads `site-out/` directly via `wrangler pages deploy`. One-time setup: `wrangler login` (or `CLOUDFLARE_API_TOKEN=…`) and `wrangler pages project create hugo-site --production-branch main`. Same bytes as k3s — no separate build config.
