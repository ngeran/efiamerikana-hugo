...
**Portrait orientation:** two landscape rules are gated to `orientation: landscape` so they don't leak into portrait, where `min-width: 768px` is satisfied by a 768–1024px-wide tablet held portrait. `.section-fill` (the viewport-fill floor) applies only in landscape — in portrait every section sizes to its content, which kills the dead bands a rotated monitor produced (1080×1920 gave `#about` a 1852px floor for 543px of content). The hero's 80/20 split is landscape-only too: `.hero-split` on the hero `<section>` is the portrait stacking hook — an unlayered `@media (orientation: portrait)` block in `input.css` §9 sets `flex-direction: column` and full-width's the aside, fixing a 5px page overflow at 768–805px wide. That block is deliberately **outside `@layer`**: layered CSS loses to Tailwind utilities like `md:flex-row`, so tidying it into a layer silently re-breaks the stacking. Full defect analysis, the measurement table, and the specificity-trap writeup live in `.claude/skills/portrait-orientation/SKILL.md`.
