...
# Compile Tailwind v4: site/src/input.css → site/static/css/main.css.
# main.css is committed and served verbatim by Hugo/Nix (NO Hugo Pipes), so run
# this after editing input.css or any layout/util class, then commit the output.
# Requires `npm install` once (devShell provides nodejs_22). For live editing,
# re-run with --watch instead of --minify in a second terminal alongside `serve`.
css:
    ./node_modules/.bin/tailwindcss -i site/src/input.css -o site/static/css/main.css --minify
